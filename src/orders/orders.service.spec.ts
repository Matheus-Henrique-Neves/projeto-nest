...
describe('OrdersService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {provide: getRepositoryToken(OrderItem), useValue: mockRepositoryOrderItem},
        {provide: getRepositoryToken(Product), useValue: mockRepositoryProduct}
      ],
    }).compile();

    service = module.get<OrdersService>(OrdersService);
    repository = module.get<Repository<Order>>(getRepositoryToken(Order));
    repositoryOrderItem = module.get<Repository<OrderItem>>(getRepositoryToken(OrderItem));
    repositoryProduct = module.get<Repository<Product>>(getRepositoryToken(Product));
  });
  ...
  describe('create', () => {
    ...
    it('deve criar um pedido', async () => {
      const createOrderItemDTO:CreateOrderItemDto = {quantity: 2, productId: 1};
      const createOrderItemDTO2:CreateOrderItemDto = {quantity: 2, productId: 3};
      const createOrderDto : CreateOrderDto={
        products: [createOrderItemDTO, createOrderItemDTO2],
        userId: 1
      }
      const produtos = [
        {id: 1, name: 'Produto1', price: 9.00},
        {id: 2, name: 'Produto2', price: 10.50},
        {id: 3, name: 'Produto3', price: 20.50}
      ];
     

      const orderCreated = {total: 58.50, userId: 1};
